...
Datum TradeUpdateFrame2(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL()) {
		...
		if (timestamp2tm(end_trade_dts_ts, NULL, tm, &fsec, NULL, NULL) == 0) {
			EncodeDateTime(tm, fsec, NULL, 0, &tzn, USE_ISO_DATES, end_trade_dts); //stefan 20210329
		}
		if (timestamp2tm(start_trade_dts_ts, NULL, tm, &fsec, NULL, NULL) ==
				0) {
			EncodeDateTime(tm, fsec, NULL, 0, &tzn, USE_ISO_DATES, start_trade_dts); //stefan 20210329
		}
