...
CREATE OR REPLACE FUNCTION CustomerPositionFrame1(
		...
		OUT status INTEGER)
RETURNS SETOF record AS '$libdir/customer_position', 'CustomerPositionFrame1'
...
CREATE OR REPLACE FUNCTION CustomerPositionFrame2(IN acct_id IDENT_T,
		...
		OUT status INTEGER, OUT symbol VARCHAR(15)[], OUT trade_id BIGINT[],
		OUT trade_status VARCHAR(10)[])
RETURNS SETOF record AS '$libdir/customer_position', 'CustomerPositionFrame2'
LANGUAGE C IMMUTABLE STRICT;
