		...
		const char *szDBPort)
{
	...
	//cout<< szConnectStr <<endl;

	sprintf(name, "%d", (int) pthread_self());
	m_Conn = new connection(szConnectStr);

    if (m_Conn->is_open()) {
        cout << "Opened database successfully: " << m_Conn->dbname() << endl;
    } else {
		...
		TSecurityDetailFrame1Output *pOut)
{
	...
	ostringstream osSQL;
//	if(pIn->access_lob_flag == 0){
//        osSQL << "SELECT * FROM SecurityDetailFrame1(0," <<
...
//              pIn->symbol << "')";
//	}else{
//        osSQL << "SELECT * FROM SecurityDetailFrame1(1," <<
//              pIn->max_rows_to_return << ",'" <<
//              pIn->start_day.year << "-" <<
//              pIn->start_day.month << "-" <<
//              pIn->start_day.day << "','" <<
